...
void setAllColor(Graph<TV, maxNodes> &g, color_node color) { // Alle Knoten als besucht bzw unbesucht markieren
    int v = g.firstVertex();
    while (v != -1) {
        g.setColor(v, color);
        v = g.nextVertex(v);
...
template <class TV, int maxNodes>
void visit(Graph<TV, maxNodes> &g, int n) { // Knoten n besuchen
    g.printVertex(n);
}

// Tiefensuche
template <class TV, int maxNodes>
int Dfs(Graph<TV, maxNodes> &g, int v, int time) {// Tiefensuche ab Knoten v
    if (!g.isVisited(v)) {
...
template <class TV, int maxNodes>
void DepthFirstSearch(Graph<TV, maxNodes> &g) { // Tiefensuche
    int time = 1;
...
// Breitensuche
template <class TV, int maxNodes>
void Bfs(Graph<TV, maxNodes> &g, int v) { // Breitensuche ab Knoten v
...
template <class TV, int maxNodes>
void BreadthFirstSearch(Graph<TV, maxNodes> &g) { // Breitensuche
    setAllColor(g, WHITE); // Alle Knoten als unbesucht markieren
    int v = g.firstVertex();
    while (v != -1) {
        if (!g.isVisited(v)) // v noch nicht besucht
